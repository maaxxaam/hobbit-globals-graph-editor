...
func parse_text(_file: String) -> Dictionary[String, Array]:
	...
	while file.get_position() < file.get_length():
		...
		line_num += 1
		if len(line) == 0:  # Wow, text file generated by exporttool with -comments option!
		...
		for i in new_entry["row_count"]:
			...
			line_num += 1
			if line.begins_with("//"):  # Wow, text file generated by exporttool with -comments option!
				line = file.get_line().strip_edges()
				line_num += 1
			while len(line) > 0:
				...
				while cur_name[len(cur_name) - string_offset] != ":":
					...
					match type_symbol:
						...
						"s":
							...
							value.append(next_value.substr(1, len(next_value) - 2))
						"g":
							fres = line.find(" ")
							var next_value: String = line.substr(0, fres)
							fres = len(line) if fres == -1 else fres
...
func parse_var_names(var_name: String, row: Dictionary) -> Array[ParsedValue]:
	...
	for item: String in row.keys():
		...
		# Weird edge case: some arrays get additional 'Guid' added somewhere, despite having baked data info
		if attr_name.contains("Guid"):
			attr_name = attr_name.replace("Guid", "")
		if attr_name.ends_with("Count"):
			attr_name = attr_name.replace("Count", "")
			value = []
			value.resize(int(row[item]))
			val_type = TYPE_ARRAY
			type = "a" # deduce array type when the first item gets inserted
		elif attr_name[len(attr_name) - 1].is_valid_int():
			var a_index_length = 1
			while attr_name.substr(len(attr_name) - a_index_length).is_valid_int():
				a_index_length += 1
			a_index_length -= 1
			array_index = attr_name.substr(len(attr_name) - a_index_length).to_int()
			attr_name = attr_name.substr(0, len(attr_name) - a_index_length)
