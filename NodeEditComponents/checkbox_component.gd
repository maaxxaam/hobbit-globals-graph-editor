...
func set_value(value: Variant):
	if value is not bool:
		if value is int and value in [0,1]:
			value = bool(value)
		elif value is float and value in [0.0, 1.0]:
			value = bool(value)
		else:
...
func init_component(node: GlobalsGraphNodeBase, var_name: String, display_name: String, value: Variant, _params: VarParameters):
